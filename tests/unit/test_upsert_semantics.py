...
def test_upsert_with_different_content_lengths():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        vault.upsert(
            "task",
            {"id": entity_id, "title": "Task", "status": "todo", "tags": []},
            content="Much longer content " * 50,
        )
...
def test_dod_content_updates_in_place():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Update content multiple times
        vault.upsert(
            "task", {"id": entity_id, "title": "My Task", "status": "todo", "tags": []}, content="Version 2 content"
        )
        vault.upsert(
            "task", {"id": entity_id, "title": "My Task", "status": "todo", "tags": []}, content="Version 3 content"
        )
        vault.upsert(
            "task", {"id": entity_id, "title": "My Task", "status": "todo", "tags": []}, content="Version 4 content"
        )
