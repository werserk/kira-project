            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if module_name.startswith("kira_plugin_") and not module_name.startswith(package_name):
                        violations.append(f"{module_path}: imports another plugin package '{module_name}'")
                    if module_name.startswith("kira.plugins"):
                        violations.append(f"{module_path}: imports from core plugins namespace '{module_name}'")
                    if is_private_core_import(module_name):
                        violations.append(f"{module_path}: imports private core module '{module_name}'")
                ...
                if module_name.startswith("kira_plugin_") and not module_name.startswith(package_name):
                    violations.append(f"{module_path}: from-imports another plugin package '{module_name}'")
                if module_name.startswith("kira.plugins"):
                    violations.append(f"{module_path}: from-imports core plugins namespace '{module_name}'")
                if is_private_core_import(module_name):
                    violations.append(f"{module_path}: from-imports private core module '{module_name}'")
