...
class ClarificationQueue:
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        high_confidence_correct = sum(1 for item in confirmed_items if item.confidence >= 0.8)

        accuracy = high_confidence_correct / total if total > 0 else 0.0

        return {
            "total_items": total,
            "by_status": by_status,
            "accuracy": accuracy,
            "avg_confidence": (sum(item.confidence for item in self._items.values()) / total if total > 0 else 0.0),
        }
