...
class TestStructuredLogger:
    ...
    def test_create_logger(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger = create_logger(name="test", log_file=log_file, level="INFO")
    ...
    def test_logger_json_format(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger.info("test_event", "Test message", correlation_id="test-123", entity_id="task-001")
...
class TestProcessingPathReconstruction:
    ...
    def test_reconstruct_ingress_to_upsert(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger.info(
                "ingress", "Event received from telegram", event_id=event_id, source="telegram", correlation_id=event_id
            )
            logger.info("validation_success", "Entity validated", entity_id=entity_id, correlation_id=event_id)
            logger.info(
                "upsert",
                "Entity created",
                entity_id=entity_id,
...
class TestMetadataLogging:
    ...
    def test_log_with_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            metadata = {"duration_ms": 42, "user": "alice", "tags": ["urgent", "bug"]}
...
class TestEndToEndLoggingScenarios:
    ...
    def test_concurrent_update_conflict_resolution(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            log_conflict(entity_id, "concurrent_update", "latest_wins", metadata={"conflicting_source": "user2"})
